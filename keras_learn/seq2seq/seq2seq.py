...
train_path = "/Users/coming/darcytech/project05_bert/datasets/THUCNews/cnews.train.txt"
...
for line in lines[:1000]:
    ...
    content_list = contents.strip().split(" ")
    title, content =content_list[0],"".join(content_list[1:])  
    train_data.append([title, content])
...
if os.path.exists('seq2seq_config.json'):
    ...
    chars = {}
    for title,content in tqdm(train_data):
    ...
    json.dump([chars,id2char,char2id], open('seq2seq_config.json', 'w'),ensure_ascii=False)
...
def data_generator():
    ...
    X,Y = [],[]
    while True:
        for title, content in train_data:
